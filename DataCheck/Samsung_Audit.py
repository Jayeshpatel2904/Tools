...
layout = [[sg.T("")], [sg.Text("Choose excel sheet with planet export: "), sg.Input(), sg.FileBrowse(key="Planet_Export")], [sg.T("")],
          [sg.T("")], [sg.Text("Choose excel sheet with DU Export"),sg.Input(), sg.FileBrowse(key="USM_DU_Export")], [sg.T("")],
          [sg.T("")], [sg.Text("Choose excel sheet with CU Export"),sg.Input(), sg.FileBrowse(key="USM_CU_Export")], [sg.T("")],
          [sg.Text("Choose a destination folder: "), sg.Input(),sg.FolderBrowse(key="-OUT-")], [sg.T("")],
          [sg.Button("Submit")], [sg.T("")],
          [sg.Text('PROGRESS')],
          [sg.ProgressBar(1, orientation='h',size=(100, 20), key='progress')]
          ]
window = sg.Window('Samsung_Audit', layout, size=(800, 400))
...
try:
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == "Exit":
            ...
            path = values["Planet_Export"]
            USM_DU_path = values["USM_DU_Export"]
            dest_path = values["-OUT-"]
            dest = dest_path + "/Audit_output-{}.xlsx".format(time)
            if dest_path == '':
                raise Exception("Please select the destination folder. ")
            template = pd.read_excel(USM_DU_path)
            CU_BEDC_template = pd.read_excel(
                USM_DU_path, sheet_name="Sheet2")
            progress_bar.UpdateBar(1, 5)
            gnb_tac_template = pd.read_excel(
                USM_DU_path, sheet_name="Sheet3")
