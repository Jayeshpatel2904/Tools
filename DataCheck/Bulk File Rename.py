...
def main():
    ...
    source_folder_entry = Entry(input_frame, width=60)
    ...
    Label(input_frame,  font=('Arial', 12, 'bold'), text="Select Destination Folder:").grid(row=1, column=0, sticky="w", padx=5)
    destination_folder_entry = Entry(input_frame, width=60)
    destination_folder_entry.grid(row=1, column=1, padx=5)
    Button(input_frame, text="Browse", font=('Arial', 12, 'bold'), command=select_destination_folder).grid(row=1, column=2, padx=5)

    # Text to be replaced
    Label(input_frame,  font=('Arial', 12, 'bold'), text="Text to Replace:").grid(row=2, column=0, sticky="w", padx=5)
    old_text_entry = Entry(input_frame, width=60)
    old_text_entry.grid(row=2, column=1, padx=5)

    # New text
    Label(input_frame,  font=('Arial', 12, 'bold'), text="New Text:").grid(row=3, column=0, sticky="w", padx=5)
    new_text_entry = Entry(input_frame, width=60)
    new_text_entry.grid(row=3, column=1, padx=5)

    # Button to Preview the changes
    Button(input_frame, text="Preview Changes",  font=('Arial', 12, 'bold'), command=populate_file_table).grid(row=4, column=1, pady=10)
    ...
    Button(root, text="Rename Files", command=rename_files, bg="green", fg="white").pack(pady=20)
