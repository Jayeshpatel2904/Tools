        ...
        with pd.ExcelWriter(os.path.join(target_folder, f'Batch_{Batch_value}.xlsx'), engine='openpyxl') as writer:
            ...
            for sheet_name, df in excel_file.items():
                if column_name in df.columns:
                    ...
                    # Create a DataFrame with just the custom header in the first row, first column
                    if custom_header:
                        header_df = pd.DataFrame({0: [custom_header]})
                    else:
                        header_df = pd.DataFrame([[''] * len(filtered_df.columns)])
                    
                    # Write the custom header first (row 1) if present
                    header_df.to_excel(writer, sheet_name=sheet_name, index=False, header=False, startrow=0)

                    # Write the table headers in row 3 (startrow=2) and the filtered data below
