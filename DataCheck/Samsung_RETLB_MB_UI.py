...
def process_file(excel_file1):
    ...
    # Apply the function
    table = check_ret_status(table)   


    def check_userlabel(row):
        return "TRUE" if row["user-label_New"] == row["user-label"] else "FALSE"

    # Assuming 'table' is your DataFrame
    table["Check User Label"] = table.apply(check_userlabel, axis=1)  # Apply the function row-wise
...
def browse_file():
    ...
    message = 'Do you have "get" Export from USM?'
...
def check_ret_status(table):
    # Ensure 'antenna-model-number' is treated as a string
    table["antenna-model-number"] = table["antenna-model-number"].astype(str)
    
    # Identify rows where 'antenna-model-number' starts with '120'
    table["Cellmax Check"] = table["antenna-model-number"].str.startswith("120")
    
    # Identify duplicates in 'o-ran-radio-unit-info/o-ran-ru-id'
    duplicates = table[table.duplicated("o-ran-radio-unit-info/o-ran-ru-id", keep=False)]["o-ran-radio-unit-info/o-ran-ru-id"]

    # Function to determine RET Status for each row
    def determine_status(row):
        if row["antenna-model-number"].startswith("120"):
            return "Good - Cellmax" if row["o-ran-radio-unit-info/o-ran-ru-id"] in duplicates.values else "Missing one Node - Cellmax"
        elif row["antenna-model-number"].startswith("MX"):
            return "Good - JMA" if row["user-label_New"] != "ERROR" else "ERROR - JMA"
        elif row["antenna-model-number"].startswith("FFVV"):
            return "Good - Commscope" if row["user-label_New"] != "ERROR" else "ERROR - Commscope"
        else:
            return "Good - Other" if row["user-label_New"] != "ERROR" else "ERROR - Other"

    # Apply the function row-wise
    table["RET Status"] = table.apply(determine_status, axis=1)
...
# Create UI window
root = tk.Tk()
root.title("LB/MB RET COMPARE V.1")
