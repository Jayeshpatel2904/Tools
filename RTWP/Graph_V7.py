...
def is_above_threshold(df_row):
    for col in columns_n70_71:
        if (df_row[col] > -90 and df_row[col] < 0) :
            return True
    return False

# # Function to plot the graph for a selected RU IP
...
#     # Plot each line (for the 4 Antennas) if the data exists
#     for col in columns_n70_71:
...
#     ax.set_title(f'RTWP Graph for RU IP: {ru_ip}')
#     ax.legend()
...
# # Function to create the UI with a tree view and color specific rows red
...
#     # Helper function to recursively set parent nodes to red
...
#                         # Mark the RU IP, Site Name, AOI, and Region red if any RU IP exceeds threshold
#                         ru_id_node = tree.insert(site_node, 'end', text=ru_ip, values=(ru_ip,), tags=('red',))
...
def create_ui(root, df):
    ...
    search_label = Label(search_frame, text="Filter by any text:")
    ...
    # Function to populate the tree with all nodes initially
    def populate_tree():
        unique_regions = df['Region'].unique()
        for region in unique_regions:
            region_node = tree.insert('', 'end', text=region)
    ...
    # Populate the tree view initially
    populate_tree()
