...
def create_ui(root, df):
    ...
    for region in unique_regions:
        region_node = tree.insert('', 'end', text=region, values=(region,), tags=('red',))
        aois = df[df['Region'] == region]['AOI'].unique()
        for aoi in aois:
            aoi_node = tree.insert(region_node, 'end', text=aoi, values=(aoi,), tags=('red',))
            sites = df[df['AOI'] == aoi]['Site Name'].unique()
            for site in sites:
                site_node = tree.insert(aoi_node, 'end', text=site, values=(site,), tags=('red',))
                ...
                for ru_ip in ru_ips:
                    ...
                    if not df_filtered.empty and df_filtered.apply(is_above_threshold, axis=1).any():
                        # Mark the text red if above threshold
                        tree.insert(site_node, 'end', text=ru_ip, values=(ru_ip,), tags=('red',))
                    else:
