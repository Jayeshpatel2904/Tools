...
def is_above_threshold(df_row):
    ...
    for col in columns_n70_71:
        if col in df_row.index and isinstance(df_row[col], (int, float)):
            if (df_row[col] > threshold and df_row[col] < 0):
                return True
    return False
...
def process_files_in_thread(threshold_value_thread, aoi_input_thread):
    ...
    try:
        ...
        # IMPORTANT: Do NOT stop the progress bar here.
        # It will be stopped by create_tree_structure's completion handler.
        # Ensure the button color and status are set by the *final* step.
...
def create_data_ui(root_widget, df):
    ...
    # Launch tree structure creation in a separate thread
...
def create_tree_structure(parent_frame, df):
    ...
    # We could switch to determinate mode here if we had precise steps for tree building
    # For now, it stays indeterminate, but it's important that *this* function is where the bar stops.
    # if main_progress_bar:
    #     root.after(0, lambda: main_progress_bar.config(mode='indeterminate')) # Ensure mode is right
    ...
    # Removed filter functions/binds as search bar is hidden
    ...
    root.after(0, lambda: load_button.config(bg="green")) # Set button green on completion
...
def run_app():
    ...
    load_button = Button(button_frame, text="Load CSV Files", command=open_file_and_process_data, bg="lightgray")
    ...
    Label(footer_frame, text="Support email: Jayeshkumar.patel@dish.com", anchor=E, fg="gray").pack(side=RIGHT, padx=10, pady=5)
