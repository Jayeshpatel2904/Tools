...
root = Tk()
root.title("RU IP Tree Viewer with Graphs")
root.geometry("1200x800")

# Define row status variable globally so it's accessible in other functions
...
def show_progress_popup():
    ...
    Label(progress_popup, text="Loading and Processing Data...").pack(pady=10)
...
def open_file():
    ...
    if df is not None:
        ...
        progress_popup, progress_bar = show_progress_popup()
        
        # Start processing in a separate thread
        threading.Thread(target=load_and_process_data, args=(df, threshold_value, progress_popup, progress_bar)).start()


# Data loading and UI creation function
def load_and_process_data(df, threshold_value, progress_popup, progress_bar):
    # Set the chunk size
    chunk_size = 1000
