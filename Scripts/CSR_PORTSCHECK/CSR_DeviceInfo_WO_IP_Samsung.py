...
class csr:
	...
	def get_mac_ip(self):
			...
			splitted_device_info = split_text_by_lines(ReadData)
			for line in splitted_device_info:
				if "0/0/0/4.205" in line:
					splitted_line = " ".join(line.split("0/0/0/4.205")[1].split()).split(" ")
					self.Alpha_TB_MAC = splitted_line[0][0:14]
			
				if "0/0/0/6.205" in line:
					splitted_line = " ".join(line.split("0/0/0/6.205")[1].split()).split(" ")
					self.Beta_TB_MAC = splitted_line[0][0:14]
		
				if "0/0/0/8.205" in line:
					splitted_line = " ".join(line.split("0/0/0/8.205")[1].split()).split(" ")
					self.Gamma_TB_MAC = splitted_line[0][0:14]
			
				if "0/0/0/5.205" in line:
					splitted_line = " ".join(line.split("0/0/0/5.205")[1].split()).split(" ")
					self.Alpha_MB_MAC = splitted_line[0][0:14]

				if "0/0/0/7.205" in line:
					splitted_line = " ".join(line.split("0/0/0/7.205")[1].split()).split(" ")
					self.Beta_MB_MAC = splitted_line[0][0:14]

				if "0/0/0/9.205" in line:
					splitted_line = " ".join(line.split("0/0/0/9.205")[1].split()).split(" ")
...
############### Loop that checks all sites in the csv list
	...
	with open(dump_file_path, 'w') as csvfile:
		...
		for site in site_list:
			...
			crt.Screen.Send("ssh " + username + "@" + site.name + "-cs000-csr001-lo10.net.prod.corp.dish-wireless.net" + chr(13))
